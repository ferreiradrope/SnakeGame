...
public class Tabuleiro extends JPanel implements ActionListener, KeyListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Ponto p : cobra.getSegmentos()) {
            g.fillRect(p.getX() * TAMANHO_BLOCO, p.getY() * TAMANHO_BLOCO, TAMANHO_BLOCO, TAMANHO_BLOCO);
        }
        g.setColor(comida.getCor());
        Ponto pc = comida.getPonto();
        g.fillRect(pc.getX() * TAMANHO_BLOCO, pc.getY() * TAMANHO_BLOCO, TAMANHO_BLOCO, TAMANHO_BLOCO);
        desenharPontuacao(g);
        if (!emJogo) {
